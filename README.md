...
本アプリケーションは、長期的な保守性と外部システム（APS等）との柔軟な連携を両立させるため、以下の設計指針を採用しています。

### クリーンアーキテクチャの採用
コードベースを **Domain (ドメイン)**, **Application (アプリケーション)**, **Infrastructure (インフラ)**, **Presentation (プレゼンテーション)** の4層に分離しています。これにより、ビジネスロジックが特定のフレームワークや外部API（Viewer API等）に依存しない構造を実現し、単体テストの容易性と変更への強さを確保しています。

### フラットスキーマ設計（疎結合）
BIMモデル側の階層構造（Project/Floor等）をシステム固有のマスタとして持たず、`Issue` モデルに直接文字列として保持する **フラットなデータ構造** を採用しています。
- **外部依存の排除**: 外部のプロジェクト構成が変更されても、DB側の制約（外部キー等）でエラーにならず、柔軟にデータを受け入れることができます。
- **情報の独立性**: 各指摘事項が自己完結した位置情報を持ち、将来的なSaaS化やマルチテナント対応が容易な設計になっています。

### CQRS（コマンド・クエリ責務分離）
複雑なバリデーションを伴う「書き込み（Command）」と、Viewer表示のために高速なレスポンスが求められる「読み取り（Query）」のロジックを分離し、要求に応じた最適な実装を選択可能にしています。

---

> [!NOTE]
> より詳細なクラス構造やシーケンス図、ER図については [doc フォルダ](./doc/) 配下の設計資料を参照してください。
